...
#######
## model building and internal validation code for event-ever model
## April 17, 2018
## Haley Hedlin
#######
...
dat$statMI <- as.numeric(dat$MI & dat$MIDY < hordy)
dat$statstrk <- as.numeric(dat$STROKE & dat$STROKEDY < hordy)
dat$statlc <- as.numeric(dat$LUNG & dat$LUNGDY < hordy)
dat$statbc <- as.numeric(dat$BREAST & dat$BREASTDY < hordy)
dat$statcrc <- as.numeric(dat$COLORECTAL & dat$COLORECTALDY < hordy)
dat$stathip <- as.numeric(dat$BKHIP.y & dat$BKHIPDY < hordy)
dat$statdeath <- as.numeric(dat$DEATH & dat$DEATHDY < hordy)

statdth <- which(dat$DEATH==1)
# loop through the eight outcomes

# note - the <= and the statMI == 0 pieces together find the people who have 
# death dates equal to their censoring time but who did not experience the 
# event of interest
## DEATHDY is never less than the event day
for(k in statdth){
  dat$statMI[k] <- ifelse(dat$DEATHDY[k] <= dat$MItime[k] & dat$statMI[k]==0, 2, 
                          dat$statMI[k])
  dat$statstrk[k] <- ifelse(dat$DEATHDY[k] <= dat$strktime[k] & 
                              dat$statstrk[k]==0, 2, dat$statstrk[k])
  dat$statlc[k] <- ifelse(dat$DEATHDY[k] <= dat$lctime[k] & dat$statlc[k]==0, 2, 
                          dat$statlc[k])
  dat$statbc[k] <- ifelse(dat$DEATHDY[k] <= dat$bctime[k] & dat$statbc[k]==0, 2, 
                          dat$statbc[k])
  dat$statcrc[k] <- ifelse(dat$DEATHDY[k] <= dat$crctime[k] & dat$statcrc[k]==0, 
                           2, dat$statcrc[k])
  dat$stathip[k] <- ifelse(dat$DEATHDY[k] <= dat$hiptime[k] & dat$stathip[k]==0, 
                           2, dat$stathip[k])
}
...
dat.fac <- cbind(AGE=dat$AGE,factor2ind(dat$ethnic2,5), DIAB=dat$DIAB, # white
                 ...
                 MItime=dat$MItime, statMI=dat$statMI, naMI=dat$naimpMI,
                 strktime=dat$strktime, statstrk=dat$statstrk, nastrk=dat$naimpstrk,
                 lctime=dat$lctime, statlc=dat$statlc, nalc=dat$naimplc,
                 bctime=dat$bctime, statbc=dat$statbc, nabc=dat$naimpbc,
                 crctime=dat$crctime, statcrc=dat$statcrc, nacrc=dat$naimpcrc,
                 hiptime=dat$hiptime, stathip=dat$stathip, nahip=dat$naimphip,
                 deathtime=dat$deathtime, statdeath=dat$statdeath, nadeath=dat$naimpdeath,
